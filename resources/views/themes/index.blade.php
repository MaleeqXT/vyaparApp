...
<head>
    ...
    <style>
        ...
        .invoice-theme-layout .inv-item-cell { display:flex; flex-direction:column; gap:2px; line-height:1.15; }
        .invoice-theme-layout .inv-item-name { font-weight:700; color:#111; }
        .invoice-theme-layout .inv-item-custom-fields { font-size:9px; font-weight:400; color:#4b5563; white-space:normal; }
        .invoice-theme-layout .inv-item-custom-fields div + div { margin-top:1px; }
...
@endforeach</div>
    ...
    <script>
        ...
        function normalizeCustomFieldLine(field) {
            ...
            if (typeof field === 'object') {
                ...
                return value ? (label ? `${escapeHtml(label)}: ${escapeHtml(value)}` : escapeHtml(value)) : escapeHtml(label);
            }
        ...
        function renderItemCell(item){
            ...
            const lines = summary
                ? summary.split('|').map(function (line) { return escapeHtml(String(line || '').trim()); }).filter(Boolean)
                : rawFields.map(normalizeCustomFieldLine).filter(Boolean);

            if (!lines.length) {
                return `<span class="inv-item-name">${baseName}</span>`;
            }

            return `<div class="inv-item-cell"><div class="inv-item-name">${baseName}</div><div class="inv-item-custom-fields">${lines.map(function (line) { return `<div>${line}</div>`; }).join('')}</div></div>`;
        }
        ...
        function persistThemeSelection(){
            ...
            try { window.localStorage.setItem(`saleInvoiceTheme:${saleId}`, signature); } catch (error) {}

            if (!themeSaveUrl || signature === lastSavedThemeSignature) {
                return;
            }

            lastSavedThemeSignature = signature;
